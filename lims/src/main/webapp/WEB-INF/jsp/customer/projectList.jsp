					...
					<div class="col-xs-12">
						...
						<form action="customer/projectList.do?project_client=${pd.project_client}" method="post" name="userForm" id="userForm">
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
								...
								<!-- 开始循环 -->
								<c:choose>
									<c:when test="${not empty projectByIdlistPage}">
											<c:forEach items="${projectByIdlistPage}" var="project" varStatus="vs">

												<tr>
													<td class='center' style="width: 30px;">${page.showCount*(page.currentPage-1)+vs.index+1}</td>
													<td class="center">${project.project_name }</td>
													<c:if test="${project.project_status == 1}">
														<td class="center">准备中</td>
													</c:if>
													<c:if test="${project.project_status == 2}">
														<td class="center">进行中</td>
													</c:if>
													<c:if test="${project.project_status == 3}">
														<td class="center">已完成</td>
													</c:if>
													<c:if test="${project.project_status == 4}">
