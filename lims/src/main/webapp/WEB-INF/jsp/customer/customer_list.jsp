					...
					<div class="row">
						...
						<table style="margin-top:20px;margin-bottom: 10px">
							<tr>
								...
								<td style="padding-left:20px;"><input class="span10 date-picker" name="lastLoginStart" id="lastLoginStart"  value="${pd.lastLoginStart}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="成立时间开始"/></td>
								<td style="padding-left:20px;"><input class="span10 date-picker" name="lastLoginEnd" id="lastLoginEnd"  value="${pd.lastLoginEnd}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="成立时间结束"/></td>
								<c:if test="${QX.cha == 1 }">
									...
									<c:forEach items="${customerList}" var="customer" varStatus="vs">
										...
										<tr>
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${customer.id }" id="${customer.client_emil }" alt="${customer.client_phone }" title="${customer.client_name }" class="ace"/><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${page.showCount*(page.currentPage-1)+vs.index+1}</td>
											<td class="center">${customer.client_number }</td>
											<td class="center"><a onclick="viewUser('${customer.id}')" style="cursor:pointer;"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i>${customer.client_name }</a></td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="editUser('${customer.id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="delUser('${customer.id }','${customer.client_name }');">
														<i class="ace-icon fa fa-trash-o bigger-120" title="删除"></i>
													</a>
													</c:if>
													<a class="btn btn-xs btn-primary" onclick="projectList('${customer.id}');">项目列表</a>
												</div>
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="editUser('${customer.id}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="delUser('${customer.id }','${customer.client_name }');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
...
//检索
function searchs(){
    var start = $("#lastLoginStart").val();	// 获取开始日期
    var end =$("#lastLoginEnd").val(); 	// 获取结束日期
