							...
							<div class="tab-content">
								<div id="home" class="tab-pane in active">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<c:if test="${var.NAME_ != 'RESULT' && var.NAME_ != 'projectid'}">
											...
											<c:if test="${var.NAME_ == 'projectid'}">
												<input type="hidden" name="projectId0" id="projectId0" value="${var.TEXT_}"/>
											</c:if>
										</tr>
									</c:forEach>
									</table>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function handle(msg){
		    //只有副经理和经理能选择
			var asssignee_ = $("#ASSIGNEE_2").val();
            if(asssignee_ != null && asssignee_ !="" && asssignee_ !="R20180131375361" && asssignee_ !="R20171231726481"){
                $("#roleSelect").tips({
                    side:3,
                    msg:'请选择经理或者副经理',
                    bg:'#AE81FF',
		...
		function getRole(){
			 ...
			 diag.Drag=true;
			 diag.Title ="选择角色";
			 diag.URL = '<%=basePath%>role/roleListWork.do?ROLE_ID=1';
			 diag.Width = 700;
			 diag.Height = 545;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
