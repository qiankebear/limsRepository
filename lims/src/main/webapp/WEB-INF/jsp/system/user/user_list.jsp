				...
				<div class="page-content">
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search" style="padding-top: 20px; padding-bottom: 10px"  >
								...
								<td style="vertical-align:top;padding-top: 20px; padding-bottom: 10px;padding-left: 10px"><input class="span10 date-picker" name="lastLoginStart" id="lastLoginStart"  value="${pd.lastLoginStart}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="最近登录开始"/></td>
								<td style="vertical-align:top;padding-top: 20px; padding-bottom: 10px;padding-left: 10px"><input class="span10 date-picker" name="lastLoginEnd" id="lastLoginEnd"  value="${pd.lastLoginEnd}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="最近登录结束"/></td>
								<td style="vertical-align:top;padding-top: 20px; padding-bottom: 10px;padding-left: 10px">
									...
									<c:forEach items="${roleList}" var="role">
										<option value="${role.ROLE_ID }" <c:if test="${pd.PARENT_ID==role.ROLE_ID}">selected</c:if>>${role.ROLE_NAME }</option>
									</c:forEach>
								  	</select>
								</td>
								<td style="vertical-align:top;padding-top: 20px; padding-bottom: 10px;padding-left: 10px">
									<select class="chosen-select form-control" name="PROSITON_CATEGORY" id="PROSITON_CATEGORY"  data-placeholder="请选择职业" style="vertical-align:top;width:98%;" >
										<option value ="">全部</option>
										<option value ="1" <c:if test="${pd.PROSITON_CATEGORY == '1'}">selected</c:if>>法医</option>
										<option value ="2" <c:if test="${pd.PROSITON_CATEGORY == '2'}">selected</c:if>>临床</option>
									</select>
								</td>
								<td style="vertical-align:top;padding-top: 20px; padding-bottom: 10px;padding-left: 10px">
									<select class="chosen-select form-control" name="POSITION_STATUS" id="POSITION_STATUS"  data-placeholder="请选择职业状态" style="vertical-align:top;width:98%;" >
										<option value ="">全部</option>
										<option value ="1" <c:if test="${pd.POSITION_STATUS == '1'}">selected</c:if>>在职</option>
										<option value ="2" <c:if test="${pd.POSITION_STATUS == '2'}">selected</c:if>>离职</option>
									</select>
								</td>
								<c:if test="${QX.cha == 1 }">
								<td style="vertical-align:top;padding-top: 20px; padding-bottom: 10px;padding-left: 10px"><a class="btn btn-light btn-xs" onclick="searchs();"  title="检索"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i></a></td>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center"><c:if test="${user.PROSITON_CATEGORY == '1'}">法医</c:if><c:if test="${user.PROSITON_CATEGORY == '2'}">临床</c:if></td>
											<td class="center"><c:if test="${user.POSITION_STATUS == '1'}">在职</c:if><c:if test="${user.POSITION_STATUS == '2'}">离职</c:if></td>
											<td class="center">${user.EMAIL}</td>
											<td class="center">${user.LAST_LOGIN}</td>
											<td class="center">${user.VISIT_TIMES}</td>
...
//检索
function searchs(){
    var start = document.getElementById("lastLoginStart").value;	// 获取开始日期
    var end = document.getElementById("lastLoginEnd").value;	// 获取结束日期
    if (end<start){	// 如果结束日期小于开始日期
        $("#lastLoginStart").tips({	// 提示用户重新输入
            side: 3,
            msg: '日期格式错误，结束日期不能小于开始日期，请重新选择日期！',
            bg: '#AE81FF',
            time: 2
        });
        return false;	// 返回不提交
