						...
						<div class="col-xs-12">
								<form action="porePlate/${msg }.do" name="userForm" id="userForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											...
											<td>
												<div class="input-group bootstrap-timepicker">
													<input readonly="readonly" class="form-control" type="text" name="operation_time" id="operation_time" value="${fn:substring(pcrRecord.operation_time, 0, 16)}" maxlength="100" placeholder="这里输入上机时间" title="成立时间" style="width:100%;"/>
										...
										<tr>
											<td style="width:130px;text-align: right;padding-top: 13px;">是否有特殊情况:</td>
											<td><textarea type="text" name="special_condition" id="special_condition" value="${pcrRecord.special_condition}" maxlength="32" placeholder="这里输入是否有特殊情况" title="特殊情况" style="width:98%;">${pcrRecord.special_condition}</textarea></td>
										</tr>
										<tr>
...
<script type="text/javascript">
    ...
    //保存
    function save(){
        if($("#instrument_info").val()==""){
            $("#instrument_info").tips({
                side:3,
                msg:'请选择仪器',
                bg:'#AE81FF',
                time:3
            });
            $("#instrument_info").focus();
            return false;
        }
        ...
        var current_procedure = 3;
        $.ajax({
            type : "POST",
            cache: false,
            url : '<%=basePath%>porePlate/completeOther',
            data: {detection_time:pcr_time,id:$("#poreId").val(),current_procedure:5,instrument_info:$("#ROLE_IDS").val(),operation_time:$("#operation_time").val(),deplane_time:$("#deplane_time").val(),special_condition:$("#special_condition").val()},
            dataType : "json",
            success: function () {
                siMenu('z150','lm144','检测待办','punching/list.do?keywords='+keyWord+'&current_procedure='+current_procedure);
                top.Dialog.close();
            }
        });
        $.ajax({
